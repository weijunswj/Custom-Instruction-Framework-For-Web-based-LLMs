...
* Stacks:User/Web/controller picks stack/thread;registry maps roles→model+reasoning/tier/speed.Defaults:G1/G2=Sol High standard;G3/recon=Luna Max priority;G4=Astra High standard;Loop=Luna Max priority;final audit=Astra Max;computer-use=>Astra Medium.Subagents resolve own stack+speed;default speed=standard;never inherit parent/root priority;priority only by explicit exact-child User/Web/controller override.No G1A/G2A/fallbacks/substitution/self-attestation.
* Launch:mirror `STACK=<name>` before prompt;executor resolves role route from registry;harness=transport only,never chooses/defaults/substitutes.New thread without stack=>ask User/Web.Worker needn't prove model;opaque metadata never gates.Unsupported route=>`ROUTE_UNAVAILABLE`;ask User/Web.
