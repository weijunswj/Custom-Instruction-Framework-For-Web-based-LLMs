...
* Models:G1/G2/G3=Luna Max;G4=Sol High.No substitution.Loop manager=Luna Max Standard/default.Spawned Luna Max MUST use service tier `priority` when selectable.Owner may choose manager `priority`;service tier alone never invalidates required Sol G4.Verify model/route;self-report nonbinding.
