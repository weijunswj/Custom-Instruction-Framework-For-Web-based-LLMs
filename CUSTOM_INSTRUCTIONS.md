...
* Stacks:User/Web/controller selects named stack per new execution thread;Toolkit registry maps Loop/recon/G1/G2/G3/G4/final-audit to exact model+model-specific reasoning/tier/speed.OpenAI default:G1/G2=Sol High standard;G3/recon=Luna Max priority;G4=Astra High standard;Loop=Luna Max priority;Astra Max final audit.No G1A/G2A/fallbacks/silent substitution/self-attestation.
* Launch:mirror `STACK=<name>` outside/before prompt;executor resolves its role route from Toolkit registry.Harness=transport only;never auto-pick/default/substitute.New thread without stack=>ask User/Web.Worker needn't prove model;opaque metadata never gates.Explicit unsupported route=>`ROUTE_UNAVAILABLE`;ask User/Web.
