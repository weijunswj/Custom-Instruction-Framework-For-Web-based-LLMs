...
* Models:G1/G2/G3=Luna Max priority;G4=Astra High standard;critical/security/final-audit G4=Astra Max standard;Loop=Luna Max;subagents=Luna Max priority.Executor follows run authority.Launch metadata outside prompt unless required;missing route=>stop+ask User/Web.No substitution/self-attestation.
...
* Managed repos:before material work,read repo Toolkit bootstrap; if absent read `https://github.com/weijunswj/ai-agent-toolkit/blob/main/repo/contracts/github-program-reconciler/web-controller-entry.md`+Parent/current children/PRs/chronology;invalid/missing required bootstrap=>`PARENT_RECONCILIATION_INCOMPLETE`.
